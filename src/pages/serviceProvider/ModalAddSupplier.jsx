...
export default function ModalAddSupplier() {
  ...
  const handleSubmit = () => {
    if (modalPage === "picture" && data?.image) {
      setModalPage("details")
    }
    else if (modalPage === "details" && data?.name && data?.phone && data?.cnic && data?.address) {
      // setModalPage("documents")
      console.log(data);
    }
    // else if (modalPage === "documents" && data?.image) {
    //   console.log(data);
  ...
  return (
    <>
      <button onClick={() => setShowModal(true)} className='w-[128px] h-[37px] bg-yellowPrimary text-white hover:ring-2 ring-inset hover:bg-white hover:text-yellowPrimary hover:ring-yellowPrimary rounded-lg text-base'>Add New</button>
      {showModal ? (
        <>
          <div
          ...
          >
            <div className="relative w-2/3 max-h- my-6 mx-auto max-w-3xl">
              ...
              <div className="border-0 rounded-3xl shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none px-8 pb-4 max-h-[92vh] overflow-hidden">
                {/*header*/}
                <button
                  className="p- ml-auto bg-transparent -mr[17px] mt-[7px] border-0 text-yellowPrimary float-right text-3xl leading-none font-semibold outline-none focus:outline-none"
                ...
                <div className="flex items-start rounded-t-3xl justify-center md:px-5 pt-3 pb-5 border-b-2 ">
                  <ol className="items-center w-full space-y-4 sm:flex sm:space-x-8 sm:space-y-0 rtl:space-x-reverse">
                    <li onClick={() => setModalPage('picture')} className="cursor-pointer flex items-center space-x-3 rtl:space-x-reverse text-yellowPrimary">
                    ...
                    <li onClick={() => setModalPage('details')} className={"flex items-center text-gray-400 space-x-3 rtl:space-x-reverse " + (modalPage !== 'picture' && 'text-yellowPrimary cursor-pointer')}>
                      <span className="hidden md:flex items-center justify-center w-9 h-9 shrink-0">
                    ...
                    {/* <li className={"flex items-center text-gray-400 space-x-3 rtl:space-x-reverse " + (modalPage === 'documents' && 'text-yellowPrimary')}>
                      <span className="hidden md:flex items-center justify-center w-9 h-9 shrink-0">
